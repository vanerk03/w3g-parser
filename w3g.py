...
NULLSTR = b'\0'
# maps may range from -MAXPOS to MAXPOS with (0, 0) at the center
MAXPOS = 16384.0
...
if sys.version_info[0] < 3:
    ...
    def b2i(b): return struct.unpack('<' + BLENFLAG[len(b)], b)[0]
    ...
    # fake lru_cache
    def lru_cache(maxsize=128, typed=False): return lambda f: f
else:
    def b2i(b): return b if isinstance(b, int) else int.from_bytes(b, 'little')
...
class ReforgedPlayerMetadata(namedtuple('ReforgedPlayerMetadata',
    ...
    def __new__(cls, player_id=-1, battle_tag='', clan='', raw=b''):
        self = super(ReforgedPlayerMetadata, cls).__new__(cls, player_id=player_id, battle_tag=battle_tag,
                                                          clan=clan, raw=raw)
        return self

    @classmethod
    def from_raw(cls, data, size=0):
        n = 0
        kw = {}
        kw['player_id'] = b2i(data[n:n+4])
        n += 4
        tg = read_until(data[n:], 34)
        kw['battle_tag'] = tg
        n += len(tg) + 1
        kw['clan'] = ''
        kw['raw'] = data[n:]
...
class AbilityPosition(Ability):
    ...
    def __str__(self):
        ...
        return '{0} at ({1:.3%}, {2:.3%} {3} {4})'.format(
            s,
            self.loc[0]/MAXPOS,
            self.loc[1]/MAXPOS,
            self.loc[0],
...
class AbilityPositionObject(AbilityPosition):
    ...
    def __eq__(self, other):
        if type(self) == type(other):
            ...
            x2, y2 = other.loc
            delta = 100
            if abs(x2 - x1) <= delta and abs(y2 - y1) <= delta and self.object == other.object:
                return True
        return False
...
class RemoveUnitFromBuildingQueue(Action):
    ...
    def __init__(self, f, player_id, action_block):
        super(RemoveUnitFromBuildingQueue, self).__init__(
            f, player_id, action_block)
...
class SomebodySetUpUsTheBomb(Action):
    ...
    def __init__(self, f, player_id, action_block):
        super(SomebodySetUpUsTheBomb, self).__init__(
            f, player_id, action_block)
...
class AllYourBaseAreBelongToUs(Action):
    ...
    def __init__(self, f, player_id, action_block):
        super(AllYourBaseAreBelongToUs, self).__init__(
            f, player_id, action_block)
...
class ChangeAllyOptions(Action):
    ...
    def __init__(self, f, player_id, action_block):
        ...
        self.flags_bits = bits(
            b2i(action_block[2:4])) + bits(b2i(action_block[5:9]))
...
class UnknownScenario1(Action):
    # unclear what it does, but prevents from parsing reforged replays properly
    size = 21
    id = 0x7A
    apm = False

    def __init__(self, f, player_id, action_block):
        super(UnknownScenario1, self).__init__(f, player_id, action_block)


class UnknownScenario2(Action):
    # unclear what it does, but prevents from parsing reforged replays properly
    size = 17
    id = 0x7B
    apm = False
    ...
    def __init__(self, f, player_id, action_block):
        super(UnknownScenario2, self).__init__(f, player_id, action_block)
...
class File(object):
    ...
    def _read_header(self):
        ...
        if hv == 0:
            ...
            raise ValueError(
                "Header must be either v0 or v1, got v{0}".format(hv))
    ...
    def _parse_startup(self, data):
        ...
        if b2i(data[offset]) != 0x19:
            ...
            while (b2i(data[offset]) == 0x39):
                ...
                offset += 1
                following_bytes = b2i(data[offset:offset+4])
                offset += 4

                if subtype == 0x3:
                    temp_player_data = ReforgedPlayerMetadata.from_raw(
                        data[offset:], following_bytes)
                    self.reforged_player_metadata.append(temp_player_data)
    ...
    def slot_record(self, pid):
        ...
        for sr in records:
            ...
            raise ValueError(
                "could not find slot record for player ID {0}".format(pid))
        return sr
...
def main():
    replay_path = "C:\\Users\\User\\Downloads\\w3g-1.0.5\\1_32_replays\\Bsk_ripley.w3g"
    f = File(replay_path)
